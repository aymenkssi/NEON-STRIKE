...
- **Rendu cartoon** : éclairage « toon » à paliers, couleurs vives. La ville est générée (`frontend/src/game/world.ts`) puis fusionnée en 2 maillages, et les collisions utilisent des boîtes invisibles. Zombies cartoon (`characters.ts`). Les armes (`weapons.ts`) sont réalistes : silhouettes de vraies armes (carcasse, chargeurs courbes, crosses, rails, organes de visée) extrudées avec des arêtes biseautées, matériaux métal / bois / polymère, main gantée.
...
- **Armurerie** (`frontend/src/game/armory.ts`, `components/Armory.tsx`) : 10 armes achetées avec les crédits, prix réglés dans la page d'administration (onglet « Boutique et messages » → Armurerie) : pistolet et fusil à pompe (gratuits), MP5, M16, M4, AK-47, sniper .50 (traverse 3 zombies), lance-grenades, minigun M134, lance-roquettes RPG-7. Le joueur emporte 4 armes en partie (son « équipement »). L'onglet Améliorations regroupe dégâts, blindage, chargeur et rechargement. Les anciennes sauvegardes gardent les armes déjà débloquées par niveau.
...
- **Modes de tir** : bouton MODE au-dessus de FIRE pour les armes qui en ont plusieurs : coup par coup, rafale de 3 balles, automatique. M16 : rafale / coup par coup ; M4 et MP5 : auto / rafale / coup par coup ; AK-47 : auto / coup par coup. Le mode choisi est mémorisé par arme pendant la partie.
